...
- **Right Panel (Financial Health):**
  - **Monthly Revenue Goal:** A bar chart filling up based on _all_ revenue streams (subscriptions + active work).
...
- **Logic:** Does not copy files. Stores _Reference Links_ to local folders, Repos, and Figma files for instant `Cmd+O` access.
