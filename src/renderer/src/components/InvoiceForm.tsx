...
const InvoiceForm: React.FC<InvoiceFormProps> = ({
  ...
  const [issueDate, setIssueDate] = useState<string>(initialData?.issueDate ? new Date(initialData.issueDate).toISOString().split('T')[0] : '');
  ...
  const handleGenerateAndSave = async () => {
    ...
    // Prepare invoice data - Note: PDF buffer generation happens in the main process via IPC
    const invoiceData: Omit<Invoice, 'id'> = {
      projectId,
      issueDate: new Date(issueDate),
      dueDate: new Date(dueDate),
      logs: selectedLogs, // Pass the logs that will be included in the invoice
      totalHours,
      totalAmount,
      currency: project.currency,
      status: 'draft', // Typically starts as draft before sending
    };
    ...
    <div className="space-y-4 p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mt-6 pt-4 border-t border-gray-200 flex justify-between items-center">
        <div className="text-lg font-semibold text-gray-900">
          Total Amount: {calculateEarnings(totalHours, project?.hourlyRate || 0).toFixed(2)} {currency}
        </div>
        <div className="flex space-x-4">
          <Button onClick={handleGenerateAndSave} disabled={!projectId || !issueDate || !dueDate || !project || selectedLogs.length === 0}>
