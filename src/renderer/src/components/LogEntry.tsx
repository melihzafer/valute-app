...
const LogEntry: React.FC<LogEntryProps> = ({ log, projectName, onEdit, onDelete }) => {
  ...
  return (
    <tr className="border-b border-border hover:bg-accent/50 transition-colors">
      <td className="py-3 px-4 text-sm text-foreground truncate" title={projectName}>
        {projectName}
      </td>
      ...
      <td className="py-3 px-4 text-sm font-medium text-foreground">
        {formatDuration(log.accumulatedTime)}
      </td>
      <td className="py-3 px-4 text-sm text-muted-foreground truncate" title={log.description}>
        {log.description || '-'}
      </td>
